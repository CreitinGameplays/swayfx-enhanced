...
When enabled, opening a window next to a focused sibling auto-splits it: wide containers split horizontally, tall containers split vertically, and the new window is placed before/after based on which half of the sibling the cursor is on. Tiling windows use a pixel border while dwindle is enabled, so no title bars are shown; toggling it back restores the configured default border. The `tabbed` and `stacking` layouts (`layout tabbed|stacking`, `workspace_layout tabbed|stacking`) are rejected while dwindle is enabled, and `layout toggle all` skips them.
