...
struct sway_container *container_create(struct sway_view *view) {
	...
	c->liquid_glass = alloc_scene_liquid_glass(c->scene_tree, 0, 0, &failed);
	wlr_scene_node_set_enabled(&c->liquid_glass->node, false);
	...
	c->liquid_glass_enabled = config->liquid_glass_enabled;
...
static void container_fullscreen_workspace(struct sway_container *con) {
	...
	sway_log(SWAY_DEBUG, "container_fullscreen_workspace: con=%p view=%p workspace=%s",
			(void*)con, con->view ? (void*)con->view : NULL,
			con->pending.workspace ? con->pending.workspace->name : "none");
...
static void container_fullscreen_global(struct sway_container *con) {
	...
	sway_log(SWAY_DEBUG, "container_fullscreen_global: con=%p view=%p",
			(void*)con, con->view ? (void*)con->view : NULL);
	con->saved_x = con->pending.x;
	con->saved_y = con->pending.y;
	con->saved_width = con->pending.width;
...
void container_fullscreen_disable(struct sway_container *con) {
	...
	sway_log(SWAY_DEBUG, "container_fullscreen_disable: con=%p view=%p mode=%d",
			(void*)con, con->view ? (void*)con->view : NULL,
...
bool container_has_liquid_glass(struct sway_container *con) {
	if (!con || !con->liquid_glass_enabled) {
		return false;
	}
	...
	if (container_get_effective_alpha(con) < 1.0f) {
		...
		pixman_region32_t *opaque = &surface->current.opaque;
		if (!pixman_region32_not_empty(opaque)) {
			transparent = true;
		} else {
			int n;
			pixman_box32_t *rects = pixman_region32_rectangles(opaque, &n);
			uint64_t opaque_area = 0;
			for (int i = 0; i < n; ++i) {
				opaque_area += (uint64_t)(rects[i].x2 - rects[i].x1) *
					(rects[i].y2 - rects[i].y1);
			}
			uint64_t total_area = (uint64_t)surface->current.width *
				surface->current.height;
			if (opaque_area < total_area * 0.95f) {
				transparent = true;
			}
		}
	}
