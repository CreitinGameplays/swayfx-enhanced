...
void layer_apply_criteria(struct sway_layer_surface *surface, struct layer_criteria *criteria) {
	if (criteria) {
		...
		surface->liquid_glass_enabled = criteria->liquid_glass_enabled;
		...
		surface->liquid_glass_enabled = false;
...
static void arrange_surface(struct sway_output *output, const struct wlr_box *full_area,
	...
	wl_list_for_each(node, &tree->children, link) {
		...
		bool liquid_glass_enabled = surface->liquid_glass_enabled;
		if (liquid_glass_enabled) {
			struct wlr_surface *wlr_surf = surface->layer_surface->surface;
			pixman_region32_t *opaque = &wlr_surf->current.opaque;
			if (pixman_region32_not_empty(opaque)) {
				...
				uint64_t total_area = (uint64_t)wlr_surf->current.width *
					wlr_surf->current.height;
...
void arrange_layers(struct sway_output *output) {
	...
	for (size_t i = 0; i < nlayers; ++i) {
		wl_list_for_each_reverse(node,
			...
			struct sway_layer_surface *surface = scene_descriptor_try_get(node,
				SWAY_SCENE_DESC_LAYER_SHELL);
			if (surface && layer_surface_is_keyboard_focusable(
	...
	wl_list_for_each(seat, &server.input->seats, link) {
		...
		if (topmost != NULL) {
			...
			// Preserve focus for interactive on-demand layers. Only clear it
			// once the focused layer stops being keyboard-focusable.
...
static struct sway_layer_surface *sway_layer_surface_create(
	...
	surface->liquid_glass_node = alloc_scene_liquid_glass(surface->tree, 0, 0, &failed);
